...
var worker = worker ?? new Worker('/js/worker.js')
worker.postMessage({ action: "init" })

// worker.addEventListener('message', event => {
//     if (event.data.action === "expanded") {
//         let expandedEvent = new CustomEvent('expanded', { detail: event.data.item })
//         document.dispatchEvent(expandedEvent)
//     }
// })
...
DBOpenRequest.onsuccess = event => utils.db = DBOpenRequest.result
...
DBOpenRequest.onupgradeneeded = event => {
    ...
    utils.db.createObjectStore(IDBSTORE, { autoIncrement: false, keyPath: 'id' })
